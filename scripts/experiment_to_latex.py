...
def make_tabular(columns: list[str], align: str, rows: list[list[str]], label: str, caption: str) -> str:
...
def generate_e29(json_path: pathlib.Path, out_dir: pathlib.Path) -> None:
    ...
    columns = ["Events", "ADL verify (ms)", "Rekor verify (ms)", "Speedup", "ADL proof (KB)", "Rekor proof (B)"]
    align = r"@{}lrrrrr@{}"
    rows = []
