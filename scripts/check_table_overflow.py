...
def _extract_braced_args(text: str, count: int | None = None) -> list[str]:
...
def extract_tables(
    ...
    for pattern in (env_pattern, standalone_tabular):
        for m in pattern.finditer(content):
            ...
            if auto_fit:
                flag = "OK"
            elif est_width > overflow_threshold:
                flag = "OVERFLOW"
            elif est_width > warning_threshold or col_count > 8:
                flag = "WARNING"
            else:
                flag = "OK"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=15.0,
        help="Width threshold in cm (default: 15.0)",
    )
    parser.add_argument(
        "--warning-threshold",
        type=float,
        default=12.0,
        help="Warning width threshold in cm (default: 12.0)",
    )
    ...
    if no_at_tables:
        ...
        print(
            color("  Suggestion: add @{} at start/end of column spec to reduce padding", "magenta")
        )
        print(color(f"{'=' * 80}", "magenta"))
