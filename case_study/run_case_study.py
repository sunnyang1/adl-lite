...
AGENTS = {
    ...
    "analyst": {
        "name": "Analyst",
        "model": "GPT-4o",
        "role": "Critical analysis of methods and results",
    },
    "writer": {"name": "Writer", "model": "Claude 3.5 Sonnet", "role": "Synthesis and writing"},
    "critic": {
        "name": "Critic",
        "model": "Claude 3.5 Sonnet",
        "role": "Quality review and fact-checking",
    },
    "coordinator": {
        "name": "Coordinator",
        "model": "GPT-4o",
        "role": "Task orchestration and delegation",
    },
}
    ...
    event_log.append(
        {
            "seq": ec[0],
            "concept_id": chain.concept_id,
            "event_type": str(etype.value),
            "actor": actor,
            "timestamp": ts(t),
            "confidence": conf if conf is not None else 0.0,
            "reasoning": reasoning,
            "status": str(chain.status.value),
            "conf_derived": chain.confidence,
        }
    )
...
def run():
    ...
    for cid, owner, desc, rels in CAPABILITIES:
        ...
        ae(
            ...
            payload={
                "model": AGENTS[owner]["model"],
                "role": AGENTS[owner]["role"],
                "l2_description": desc,
                "l3_relations": [{"predicate": p, "target": tg} for p, tg in rels],
            },
            t=t,
        )
        t += 3
    ...
    validations = {
        "literature-search": [
            ("analyst", 0.82, "Found 47 relevant papers"),
            ("coordinator", 0.75, "Covers expected databases"),
        ],
        "citation-network-analysis": [
            ("analyst", 0.78, "Identifies seminal papers correctly"),
            ("coordinator", 0.70, "Graph construction validated"),
        ],
        "keyword-extraction": [
            ("analyst", 0.85, "Keywords align with domain"),
            ("writer", 0.72, "Useful for section organization"),
        ],
        "methodology-evaluation": [
            ("critic", 0.88, "Framework covers validity threats"),
            ("coordinator", 0.79, "Scores correlate with manual assessment"),
        ],
        "statistical-validation": [
            ("critic", 0.91, "Caught 3 p-hacking papers"),
            ("writer", 0.65, "Cross-check limited by statistical expertise gap"),
        ],
        "gap-identification": [
            ("writer", 0.74, "Gaps align with narrative needs"),
            ("coordinator", 0.71, "Actionable for planning"),
        ],
        "trend-detection": [
            ("critic", 0.45, "Flaw: 1-year windows miss multi-year cycles"),
            ("coordinator", 0.52, "Partially validated, window concern noted"),
        ],
        "abstract-generation": [
            ("critic", 0.62, "Overstatement rate 40% on 25-paper test"),
            ("coordinator", 0.68, "Well-structured but needs calibration"),
        ],
        "section-synthesis": [
            ("critic", 0.77, "Coherent narratives with hedging"),
            ("analyst", 0.80, "Technical accuracy maintained"),
        ],
        "figure-creation": [
            ("analyst", 0.73, "Accurately represents patterns"),
            ("coordinator", 0.69, "Meets publication standards"),
        ],
        "reference-management": [
            ("critic", 0.86, "Zero formatting errors in 50-paper test"),
            ("scout", 0.78, "Deduplication correctly merges 12 entries"),
        ],
        "logical-consistency-check": [
            ("analyst", 0.84, "Caught 5 contradictions"),
            ("writer", 0.71, "Feedback improved revision quality"),
        ],
        "citation-verification": [
            ("scout", 0.79, "Verified 92% of citations"),
            ("analyst", 0.76, "Caught 3 misattributed claims"),
        ],
        "overstatement-detection": [
            ("writer", 0.66, "Suggestions improved accuracy"),
            ("coordinator", 0.60, "Acceptable detection, needs FP tuning"),
        ],
        "methodology-critique": [
            ("analyst", 0.83, "Aligns with manual review"),
            ("writer", 0.69, "Appropriate detail for revision"),
        ],
        "task-delegation": [
            ("writer", 0.76, "Assignments match capabilities"),
            ("analyst", 0.74, "Balanced workload"),
        ],
        "quality-gating": [
            ("critic", 0.81, "Thresholds prevent low-quality propagation"),
            ("writer", 0.72, "Rejection feedback helps"),
        ],
    }
    for cid, vs in validations.items():
        for validator, conf, reason in vs:
            ...
            ae(
                ...
                conf=conf,
                reasoning=reason,
                payload={"model": AGENTS[validator]["model"]},
                t=t,
            )
    ...
    evidence = [
        (
            ...
            "scout",
            0.88,
            "Retrieved 47 papers. P@10=0.90, recall~0.72.",
            {"papers": 47, "p_at_10": 0.90, "recall": 0.72},
        ),
        (
            "methodology-evaluation",
            "analyst",
            0.85,
            "Evaluated 25 papers. Cohen kappa=0.71.",
            {"papers": 25, "kappa": 0.71},
        ),
        (
            "abstract-generation",
            "writer",
            0.58,
            "25 abstracts. Overstatement rate: 40% (10/25). Severity 1.8/3.",
            {"abstracts": 25, "overstatement": 0.40, "severity": 1.8},
        ),
        (
            "citation-verification",
            "critic",
            0.82,
            "143 citations checked. 8 misattributions (5.6%). TP=0.94.",
            {"citations": 143, "errors": 8, "tp": 0.94},
        ),
        (
            "trend-detection",
            "analyst",
            0.40,
            "5 trends, 3 false reversals. 1-year window too sensitive.",
            {"trends": 5, "false_reversals": 3, "window": "1-year"},
        ),
        (
            "section-synthesis",
            "writer",
            0.79,
            "25 papers, 4 clusters. Coherence=4.2/5, accuracy=4.0/5.",
            {"papers": 25, "clusters": 4, "coherence": 4.2},
        ),
        (
            "gap-identification",
            "analyst",
            0.72,
            "7 gaps found. 5 novel, 2 already addressed. Precision=0.71.",
            {"gaps": 7, "novel": 5, "precision": 0.71},
        ),
        (
            "logical-consistency-check",
            "critic",
            0.81,
            "12 sections. 5 contradictions, 3 unsupported. FPR=0.08.",
            {"sections": 12, "contradictions": 5, "fpr": 0.08},
        ),
        (
            "quality-gating",
            "coordinator",
            0.77,
            "8 outputs gated. 2 rejected, both justified. Precision=1.0.",
            {"gated": 8, "rejected": 2, "precision": 1.0},
        ),
        (
            "statistical-validation",
            "critic",
            0.87,
            "15 papers. 5 flagged, 4 confirmed. Precision=0.80.",
            {"papers": 15, "flagged": 5, "confirmed": 4, "precision": 0.80},
        ),
    ]
    ...
    ae(
        ...
        payload={"false_reversals": 0, "detected": 4, "window": "3-year"},
        t=t,
    )
    ...
    ae(
        ...
        payload={"parent": "abstract-generation", "target_overstatement": 0.10},
        t=t,
    )
    ae(
        chains["abstract-generation"],
    ...
    ae(
        ...
        payload={"overstatement": 0.08, "quality": 4.1},
        t=t,
    )
    ae(
        cal,
    ...
    ae(
        ...
        payload={"predicate": "co-occurs-with", "target": "methodology-critique"},
        t=t,
    )
    t += 2
    ae(
        chains["literature-search"],
        EventType.RELATE,
        "coordinator",
        conf=0.85,
        reasoning="literature-search feeds into reference-management pipeline.",
        payload={"predicate": "depends-on", "target": "reference-management"},
        t=t,
    )
    ...
    more = [
        (
            ...
            "scout",
            0.76,
            "47 papers. 3 seminal, 7 clusters. Expert overlap 85%.",
            {"papers": 47, "clusters": 7, "overlap": 0.85},
        ),
        (
            "keyword-extraction",
            "scout",
            0.81,
            "156 keywords from 47 abstracts. Expert kappa=0.78.",
            {"keywords": 156, "kappa": 0.78},
        ),
        (
            "figure-creation",
            "writer",
            0.71,
            "8 figures. Clarity=4.0/5, accuracy=3.8/5.",
            {"figures": 8, "clarity": 4.0},
        ),
        (
            "reference-management",
            "writer",
            0.84,
            "143 refs. 12 dups caught. Zero format errors.",
            {"refs": 143, "dups": 12},
        ),
        (
            "methodology-critique",
            "critic",
            0.80,
            "15 papers. 23 threats, 19 valid. Precision=0.83.",
            {"papers": 15, "threats": 23, "precision": 0.83},
        ),
        (
            "task-delegation",
            "coordinator",
            0.74,
            "30 tasks, 4 agents. Zero delegation errors.",
            {"tasks": 30, "errors": 0},
        ),
        (
            "overstatement-detection",
            "critic",
            0.69,
            "12 sections. 18 flagged: 14TP, 4FP. Recall=0.82.",
            {"flagged": 18, "tp": 14, "recall": 0.82},
        ),
    ]
...
def compute_stats(chains, elog):
    ...
    return {
        ...
        "confidence": {
            "min": round(min(cvs), 3),
            "max": round(max(cvs), 3),
            "mean": round(sum(cvs) / len(cvs), 3),
        }
        if cvs
        else {},
        "deprecated": deprecated,
        "forks": etc.get("fork", 0),
        "relates": etc.get("relate", 0),
