...
The zero integrity failures across 201 chains in E6 confirm that SHA-256 hashing operates without degradation on real-world data. The longest chain (300 events) verified without degradation, and aggregate throughput of 31,100 events per second indicates that chain operations are not the bottleneck---CSV parsing and \texttt{Event} materialisation consume the majority of wall-clock time.
...
\textbf{Confidence calibration.} The confidence value in a \texttt{VALIDATE} event is self-reported by the actor (agent or human) as a payload field. Algorithm~2 aggregates these values under the assumption that they reflect calibrated probabilities, but cognitive-science and LLM-evaluation literature consistently demonstrate overconfidence: agents frequently report $\mathit{confidence} = 0.99$ when their actual accuracy is substantially lower. ADL Lite currently has no mechanism to detect or correct miscalibrated reporters. A per-actor Expected Calibration Error (ECE) tracker, or a meta-validator agent that audits historical accuracy against ground-truth labels, would be required before the aggregated confidence can be interpreted as a reliable probability. Without such calibration, the \texttt{confidence} field should be treated as a voteweight expression rather than a statistically valid probability.

\textbf{Unstructured agent communication.} L2 (Markdown body) imposes no structure on how agents express reasoning. An agent may write circular arguments, omit premises, or use vague referents (``the aforementioned mechanism'') that evade pronoun-based SSA checks. Current L3/L4 validation inspects structured blocks only; it does not evaluate the logical quality of prose in L2. Fixed reasoning templates (e.g., \texttt{[Problem:\ X, Approach:\ Y, Evidence:\ Z]}) would provide stronger guarantees than lexical restrictions, but no template mechanism is implemented in Phase~1.

\textbf{Append-only accumulation and concept redundancy.} The append-only design treats every event as a permanent fact. This aligns with the Wittgensteinian grounding but creates three practical problems in long-running multi-agent deployments. \emph{(i)~Junk accumulation:} agents may append speculative, low-quality, or later-disproven events that remain in the chain forever, increasing storage and derivation cost. \emph{(ii)~Concept redundancy:} two agents may describe the same underlying concept with different terminology; without automatic semantic-similarity detection, the system registers both as independent concepts. \emph{(iii)~Preference drift:} user or organisational preferences change over time, yet the chain preserves all historical preferences, forcing derivation algorithms to consider stale context. Phase~2 directions to address these issues include: (a)~a \emph{quality gate} that scores information gain before appending events to the main chain, routing low-value events to a draft side-chain; (b)~embedding-based semantic deduplication that flags candidate \texttt{isomorphic-to} links when concept similarity exceeds a threshold; and (c)~a \emph{controlled forgetting} model in which old events are physically archived (retaining hash-link integrity for audit) while the active derivation window considers only recent events, enabling preference management without unbounded chain growth.
