...
\item
  \textbf{Field ordering.} Fields are serialized in the fixed order: \(\text{event\_id}\), \(\text{concept\_id}\), \(\text{event\_type}\), \(\text{actor}\), \(\text{reasoning}\), \(\text{payload}\), \(\text{previous\_event\_id}\). The \(\text{timestamp}\) field is \textbf{excluded} from the hash input (see Section 3.6 for the clock-skew rationale). The \(\text{hash}\) field is naturally excluded because it is the value being computed.
\item
...
\textbf{Proposition 1 (Hash Determinism).} For any two event records \(e_1\) and \(e_2\), if \(e_1 \setminus \{\text{hash}, \text{timestamp}\} = e_2 \setminus \{\text{hash}, \text{timestamp}\}\), then \(H(e_1) = H(e_2)\).
...
\textbf{Proposition 3 (Timestamp Independence).} The hash of an event does not depend on its \(\text{timestamp}\) field.

\emph{Proof.} The \(\text{timestamp}\) field is explicitly removed by the set subtraction \(e \setminus \{\text{hash}, \text{timestamp}\}\) prior to serialization. Therefore, two events that differ only in their timestamp values produce identical byte sequences \(b\) and hence identical hashes. \(\square\)

\textbf{Remark on hash chain integrity.} The exclusion of \(\text{timestamp}\) from the hash input is a deliberate design choice. Event ordering is determined entirely by the append sequence (i.e., the \(\text{previous\_event\_id}\) linkage), not by timestamp values. This ensures that clock skew---whether benign (agent with misconfigured clock) or adversarial (intentional backdating)---cannot cause hash mismatches or chain forks. The timestamp field serves informational purposes only: human readability, coarse temporal queries, and audit log display. It is not used for verification or ordering.
...
\textbf{Timestamp exclusion from the hash input.} The \(\text{timestamp}\) field of every event is explicitly excluded from the SHA-256 hash computation, as specified in Section 3.3.1. Formally, for any event \(e\):
...
\textbf{Non-monotonic timestamp tolerance.} Because timestamps are not used for ordering or hashing, ADL Lite tolerates arbitrary non-monotonicity in timestamp values. An event may carry a timestamp earlier than its predecessor, later than its successor, or any other value, without affecting chain validity. This is essential for distributed settings where agents may have unsynchronized clocks.
...
\textbf{Corollary 2 (Sybil Resistance).} A single actor cannot increase confidence by submitting multiple \(\text{VALIDATE}\) events.
