...
The unstructured **plain-LLM** arm-documented separately under **`plain_llm`**-reports pooled judge means for the plain writings of **3.0**, with mean ADL-minus-plain-LLM deltas **1.0667** and **1.6000** respectively, yielding a between-judge mean delta of approximately **1.3334**. Judge disagreement thresholds (delta >= **2**) register **five** disputed rows specifically on the plain-LLM strand (`plain_llm_judge_disagreement_count`). Across the fifteen ADL/fair-plain rows, adjudicators disagree widely on only **one** discovery (`disagreement_count` **1** in the aggregate summary).
...
In sum, the Phase B aggregates establish measurable retrieval uplift under explicit L3 signal, mechanically traceable consensus events, deterministic scope denial on curated probes, and mixed ambiguity outcomes depending on fairness controls-while underscoring LLM adjudication, fixture reuse, retrieval ablations, and harness heterogeneity as open threats to external validity pending Track 3 consolidation.
...
These findings should be interpreted as phase-scoped pilot results, not final performance claims. The retrieval ablation relies on a small L3-only subset (`q21-q25`), so the strongest RQ3 lift arises from five relation-sensitive probes. The current RQ1 judge pass uses proxy LLM adjudication rather than human rating, which supports reproducible consistency checks but cannot substitute for human-grounded annotation. Reported judge statistics are also based on a bounded discovery set (`n=15`) and include fixture-backed components in the plain-LLM path when live API-backed reruns are unavailable. More broadly, the AML corpus and scripted workflows are designed for controlled reproducibility, not for immediate operational deployment claims.
