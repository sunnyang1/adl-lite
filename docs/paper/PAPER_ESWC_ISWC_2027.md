...
**Contribution 3: Throughput validation and correctness at scale.** This paper reports the results of eleven experiments establishing correctness properties across the full system: 100% chain integrity verification on 50 valid and 10 intentionally corrupted chains (E1); 100% status derivation accuracy across 2,204 exhaustive event sequences including 2,197 three-event combinations and 7 edge cases (E2); 100% snapshot round-trip consistency across 38 concept files spanning synthetic and real AML domains (E3); 100% precondition enforcement precision and recall on 13 action-validation test cases (E4); 100% multi-agent auditability across 5-agent simulations producing independently verifiable chains (E5); 100% chain integrity at scale on 201 chains from real IBM AML transaction data (E6); real-time pattern detection on actual AML transactions with zero false positives across 9,000 legitimate events (E7); edge offline synchronization with zero merge conflicts (E8); Git baseline comparison (E9); full FDE pipeline validation with 201/201 concepts passing executor validation (E10); and side-effect stress testing with 1,000 enqueued effects (E11). These experiments validate system correctness and throughput scalability; they do not constitute domain-level evaluation of ontological quality (e.g., human expert ratings of concept usefulness) or operational compliance certification. The aggregate evidence supports the claim that event-first derivation of ontological state is computationally feasible, cryptographically sound, and empirically correct under exhaustive test conditions.
...
**Table: Empirical comparison of nanopublications with Trusty URIs and ADL Lite EventChains.** Nanopublication figures are derived from published dataset analyses [^40^][^41^][^42^].
...
| Integrity mechanism | Content-addressed SHA-256 hash in URI | Cryptographic hash chain (`previous_event_id` linkage) |
| Structural overhead | 31--86% of triples are non-assertion (head, provenance, pub-info) | ~200 bytes/hash per event (<1% for typical chains) |
| Infrastructure | RDF/SPARQL triplestore + nanopub server network | Markdown files in standard Git repositories |
| Verification time | O(1) per nanopub (hash → URI comparison) | O(n) per chain (sequential hash verification) |
| Lifecycle governance | None (status is publication metadata, not derived) | Deterministic status derivation + precondition rules |
...
1. **Field ordering.** Fields are serialized in the fixed order: $\text{event\_id}$, $\text{concept\_id}$, $\text{event\_type}$, $\text{actor}$, $\text{reasoning}$, $\text{timestamp}$, $\text{payload}$, $\text{previous\_event\_id}$. The $\text{hash}$ field is naturally excluded because it is the value being computed.
...
**Proposition 1 (Hash Determinism).** For any two event records $e_1$ and $e_2$, if $e_1 \setminus \{\text{hash}\} = e_2 \setminus \{\text{hash}\}$, then $H(e_1) = H(e_2)$.
...
**Timestamp inclusion in the hash input.** The $\text{timestamp}$ field of every event is explicitly included in the SHA-256 hash computation, as specified in Section 3.3.1. Formally, for any event $e$:

$$
H(e) = \mathsf{SHA\text{-}256}\bigl(\mathsf{serialize\_canonical}(e \setminus \{\text{hash}\})\bigr)
$$

This means that two events that differ only in their $\text{timestamp}$ values produce different hashes. Any post-hoc timestamp edit therefore breaks the cryptographic chain at the next event link, making timestamp tampering detectable during `verify_integrity()`. Clock skew tolerance is achieved not by excluding timestamps from the hash, but by defining event ordering exclusively through the `previous_event_id` cryptographic linkage, which is independent of wall-clock time.
...
**Corollary 2 (Bounded Confidence Aggregation per Distinct Actor).** A single actor cannot increase confidence by submitting multiple $\text{VALIDATE}$ events. Full Sybil resistance---preventing an adversary from creating arbitrarily many distinct actor identities to inflate confidence---requires cryptographic actor authentication (Phase 3; Section 3.7). In Phase 1, the bound provides Sybil *awareness* (redundant events from the same actor string do not compound) but not Sybil *resistance* under adversarial identity fabrication.
...
This section describes the design of eleven experiments (E1--E11) that evaluate ADL Lite across five dimensions: foundational correctness of the EventChain data structure (E1--E3), enforcement fidelity of the ActionExecutor precondition system (E4), multi-agent auditability (E5), baseline comparison against Git-only integrity checking (E9), scale validation on a real-world financial dataset (E6), real-time pattern detection (E7), edge offline synchronization (E8), full FDE pipeline validation (E10), and side-effect stress testing (E11). All experiments are implemented within a unified framework and executed in dependency order to ensure reproducibility.
...
| Dataset | IBM AML HI-Small\_Trans.csv (9,300 rows, 201 unique accounts) [^1] |
...
**Chain length distribution.** The 201 chains exhibit a length distribution ranging from 2 events (newly registered accounts with minimal activity) to 300 events (the most active account in the dataset).
...
| E6 --- IBM AML Pipeline | passed | 299 ms | **201/201** chains integrity OK |
...
E6 processes the IBM AML HI-Small dataset, importing 201 accounts as `EventChain` instances from 9,300 transaction rows. The mean chain length is 46.3 events per account; the median is 10 events. The full chain length distribution is reported in Table 8d (Section 4.8) and summarised here: the 25th percentile is $\sim$5 events, the 75th percentile is $\sim$20 events, the 95th percentile is $\sim$100 events, and the single maximum is 300 events (account `acct-A0990`).

**Chain integrity at scale.** All 201 chains pass `verify_integrity()` with zero failures, including the 1 suspicious-account chain that contains laundering-flagged events. The suspicious-account subset also achieves **1/1** integrity OK. This demonstrates that the cryptographic linking overhead (SHA-256 hashing plus `previous_event_id` validation) scales linearly and imposes no measurable degradation even on the longest observed chain.
...
**Real-world laundering distribution.** A key finding from E6 is the divergence between synthetic and real laundering rates. Injected synthetic patterns (e.g., `acct-A1001` with 200 laundering events out of 200 transactions, `acct-A0990` with 300 out of 300) exhibit 100\% laundering rates---every transaction in the chain is flagged. In contrast, the real IBM dataset shows a laundering rate of **3.23\%** (300 laundering events out of 9,300 total transactions). The most extreme example is account `acct-A0990`, which contains 300 laundering events across 300 transactions---a laundering rate of 100\%. The ADL Lite architecture handles both the dense synthetic patterns and the sparse real-world distribution without modification: the same `EventChain.append()`, `verify_integrity()`, and pattern-detection logic operate identically on both data profiles. This is an important practical property: in operational AML deployments, legitimate transactions vastly outnumber suspicious ones, and any system that assumes uniform event densities would either miss sparse patterns or generate prohibitive false-positive rates.

**Processing throughput.** The wall-clock runtime for E6 is 299 ms. The processing throughput is approximately **31,100 events per second** (9,300 events $\div$ 0.299 s). Profiling indicates that the bottleneck is CSV row parsing and `Event` object instantiation, not chain operations or SHA-256 hashing. At this throughput, a 10,000-event dataset completes in under 1 second on a single workstation without GPU acceleration or distributed processing, suggesting that the architecture is suitable for batch-processing production workloads an order of magnitude larger on modest hardware. Full 5-million-event benchmarks are future work (Section 6.5).
...
The zero integrity failures across 201 chains in E6 confirm that SHA-256 hashing operates without degradation on real-world data. The longest chain (300 events) verified without degradation, and aggregate throughput of 31,100 events per second indicates that chain operations are not the bottleneck—CSV parsing and `Event` materialisation consume the majority of wall-clock time.
...
**Confidence calibration.** The confidence value in a `VALIDATE` event is self-reported by the actor (agent or human) as a payload field. Algorithm 2 aggregates these values under the assumption that they reflect calibrated probabilities, but cognitive-science and LLM-evaluation literature consistently demonstrate overconfidence: agents frequently report *confidence* = 0.99 when their actual accuracy is substantially lower. ADL Lite currently has no mechanism to detect or correct miscalibrated reporters. A per-actor Expected Calibration Error (ECE) tracker, or a meta-validator agent that audits historical accuracy against ground-truth labels, would be required before the aggregated confidence can be interpreted as a reliable probability. Without such calibration, the `confidence` field should be treated as a voteweight expression rather than a statistically valid probability.

**Unstructured agent communication.** L2 (Markdown body) imposes no structure on how agents express reasoning. An agent may write circular arguments, omit premises, or use vague referents ("the aforementioned mechanism") that evade pronoun-based SSA checks. Current L3/L4 validation inspects structured blocks only; it does not evaluate the logical quality of prose in L2. Fixed reasoning templates (e.g., `[Problem: X, Approach: Y, Evidence: Z]`) would provide stronger guarantees than lexical restrictions, but no template mechanism is implemented in Phase 1.

**Append-only accumulation and concept redundancy.** The append-only design treats every event as a permanent fact. This aligns with the Wittgensteinian grounding but creates three practical problems in long-running multi-agent deployments. *(i) Junk accumulation:* agents may append speculative, low-quality, or later-disproven events that remain in the chain forever, increasing storage and derivation cost. *(ii) Concept redundancy:* two agents may describe the same underlying concept with different terminology; without automatic semantic-similarity detection, the system registers both as independent concepts. *(iii) Preference drift:* user or organisational preferences change over time, yet the chain preserves all historical preferences, forcing derivation algorithms to consider stale context. Phase 2 directions to address these issues include: (a) a *quality gate* that scores information gain before appending events to the main chain, routing low-value events to a draft side-chain; (b) embedding-based semantic deduplication that flags candidate `isomorphic-to` links when concept similarity exceeds a threshold; and (c) a *controlled forgetting* model in which old events are physically archived (retaining hash-link integrity for audit) while the active derivation window considers only recent events, enabling preference management without unbounded chain growth.
...
This paper presented ADL Lite, an event-first operational ontology for multi-agent concept consensus, and evaluated it through eleven experiments establishing correctness properties across chain integrity, status derivation, snapshot round-trip consistency, precondition enforcement, multi-agent auditability, and scalability on real-world financial transaction data. The four principal contributions are restated below with the quantitative results obtained.

The event-first ontology architecture was validated on 201 EventChains constructed from the IBM AML HI-Small dataset sample, comprising 9,300 events with an average chain length of 46.3 events and a maximum of 300. Zero integrity failures were observed across all chains, including 1 suspicious-account chain containing laundering-flagged events. Evaluation on full 5-million-event datasets is future work. The 100% derivation accuracy demonstrated in E2 (2,204/2,204 exhaustive event sequences) confirms that status, confidence, and validator identity can be computed reliably from append-only cryptographic chains, eliminating the stored-state consistency bugs endemic to object-first designs [^3^][^14^].
...
## C. Formal Semantics of EventChains

This appendix provides the complete formal foundation for the derivation semantics, cryptographic integrity, and fork-confluence properties used in Section 3.9 of the main paper. All definitions, theorems, and proof sketches are self-contained.

### C.1 Event Alphabet

Let Σ denote the set of all event types in ADL Lite. The alphabet is partitioned into *lifecycle* events Σ_life, which affect concept status, and *communication* events Σ_comm, which do not:

- Σ_life = {REGISTER, VALIDATE, DEPRECATE, FORK, ARCHIVE}
- Σ_comm = {RELATE, EVIDENCE, SEAL, ANNOUNCE, PUBLISH, SNAPSHOT}
- Σ = Σ_life ∪ Σ_comm

An *event* is a tuple e = (τ, a, t, p, h, h_prev, e_prev) where:

- τ ∈ Σ is the event type;
- a ∈ 𝓐 is the actor identifier;
- t ∈ ℝ≥0 is the timestamp (ISO-8601 serialised);
- p ∈ 𝓟 is the payload, a JSON-encodable dictionary;
- h ∈ {0,1}^256 is the SHA-256 hash of the event's canonical content;
- h_prev ∈ {0,1}^256 ∪ {genesis} is the hash of the preceding event (or "genesis" for the first event);
- e_prev ∈ ID ∪ {null} is the unique identifier of the preceding event.

### C.2 Well-Formedness Predicate

An *EventChain* C for concept c is a finite sequence of events C = [e₁, e₂, ..., eₙ] satisfying the well-formedness predicate WF(C):

1. **Genesis anchoring.** e₁.h_prev = "genesis" and e₁.e_prev = null.
2. **Concept consistency.** Every event in C has the same concept_id, namely c.
3. **Cryptographic linkage.** For all i ∈ {2, ..., n}: eᵢ.h_prev = eᵢ₋₁.h and eᵢ.e_prev = eᵢ₋₁.event_id.
4. **Hash correctness.** For all i ∈ {1, ..., n}: eᵢ.h = SHA-256(Canon(eᵢ)), where Canon(e) is the canonical JSON serialization with keys sorted lexicographically, floats rounded to 6 decimal places, and all fields (including timestamp t) included.
5. **Identifier uniqueness.** All event_id values within C are pairwise distinct.

The canonicalization function Canon(·) recursively rounds floats to 6 decimal places, sorts object keys, and serialises as UTF-8 JSON.

**Remark on timestamp inclusion.** The timestamp t is explicitly included in the hash input. This ensures that two otherwise-identical events created at different times produce different hashes, preserving tamper-evidence for provenance. Event *ordering* within a chain is determined by cryptographic linkage (h_prev), not by timestamp comparison, so clock skew does not affect chain validity.

### C.3 Status Derivation Function δ

The status derivation function δ: 𝓒 → S maps a well-formed EventChain to a status value in the status space S = {provisional, validated, deprecated, forked, archived}.

For a chain C = [e₁, ..., eₙ], define the *lifecycle subsequence*:

C_life = [e ∈ C | e.τ ∈ Σ_life]

Then δ is defined:

δ(C) = provisional                         if C_life = []
       f(τ_last)                           otherwise

where τ_last is the event type of the last element of C_life, and f: Σ_life → S is the total function:

- f(REGISTER)  → provisional
- f(VALIDATE)  → validated
- f(DEPRECATE) → deprecated
- f(FORK)      → forked
- f(ARCHIVE)   → archived

This formalizes Algorithm 1 from Section 3.9.

### C.4 Confidence Aggregation Function γ

The confidence aggregation function γ: 𝓒 → [0, 1] maps a well-formed EventChain to a confidence score. Let V = [e ∈ C | e.τ = VALIDATE] be the subsequence of validation events.

γ(C) = 0.0                                             if V = []
       min(1.0, c_base + 0.05 × (N_vals − 1))          otherwise

where:

- actors(V) = {e.a | e ∈ V}
- φ(a, V) = max {e.p.confidence | e ∈ V ∧ e.a = a}
- c_base = max(0.5, mean({φ(a, V) | a ∈ actors(V)}))
- N_vals = |actors(V)|

The function φ(a, V) selects the highest confidence value submitted by actor a, ensuring that duplicate validations from the same actor do not compound. The base confidence c_base is the mean of per-actor maxima, floored at 0.5. An additive bonus of 0.05 per additional distinct validator (beyond the first) is applied, capped at 1.0.

This formalizes Algorithm 2 from Section 3.9.

### C.5 Fork and Confluence Semantics

A *fork* transforms a concept C into a pair (C_fork, C′) where C_fork is the original concept with a FORK event appended, and C′ is a new concept with a fresh identifier and genesis event.

**Definition (Fork).** Let C = [e₁, ..., eₙ] be a well-formed chain with δ(C) ≠ forked. A fork by actor a with timestamp t and payload p produces:

- e_FORK = (FORK, a, t, p, h, h_prev, e_prev)
- C_fork = C ⊕ [e_FORK]
- e′_REG = (REGISTER, a, t, p′, h′, genesis, null)
- C′ = [e′_REG]

where h_prev = eₙ.h, e_prev = eₙ.event_id, h = SHA-256(Canon(e_FORK)), and h′, p′ are the hash and payload of the new concept's registration event.

**Definition (Confluence).** Two chains C₁ and C₂ are *confluent* if they share a common prefix P such that C₁ = P ⊕ S₁ and C₂ = P ⊕ S₂, and the first events of S₁ and S₂ (if non-empty) both have h_prev equal to the hash of the last event in P.

Confluence captures the notion that two agents can independently extend the same chain without conflict, provided their extensions form valid cryptographic continuations of the shared prefix. The CRDT merge operation (Section 3.6, §6.5) preserves confluence by taking the union of all events and topologically sorting by the partial order induced by e_prev links.
...
#### C.6.1 Theorem 1 (Determinism)

**Statement.** For any well-formed EventChain C, δ(C) returns a unique status s ∈ S that depends only on the last lifecycle event in C.

*Proof sketch.* By definition of δ (Section C.3), the derivation proceeds in three deterministic steps: (i) filter C to C_life using the fixed predicate τ ∈ Σ_life; (ii) if C_life is empty, return the constant provisional; (iii) otherwise extract the last element of C_life and apply the total function f. Each step is a deterministic function of C. The last element of a finite ordered sequence is unique. Therefore δ(C) is unique for any C. ∎

#### C.6.2 Corollary 1 (Status Stability)

**Statement.** Appending a non-lifecycle event to C does not change δ(C).

*Proof sketch.* A non-lifecycle event has τ ∉ Σ_life, so it is excluded by the filter in the definition of C_life. Therefore C_life is unchanged, τ_last is unchanged, and by Theorem 1, δ(C) is unchanged. ∎

#### C.6.3 Theorem 2 (Confluence under Fork)

**Statement.** Let C be a well-formed chain. If C forks, producing C_fork = C ⊕ [e_FORK] and a new chain C′ = [e′_REG] ⊕ C_new, then:

1. δ(C_fork) = forked; and
2. δ(C′) = provisional regardless of C and C_new.

*Proof sketch.* (1) The FORK event is a lifecycle event (FORK ∈ Σ_life). By Theorem 1, δ(C_fork) depends only on the last lifecycle event in C_fork. Since e_FORK is appended after all events in C, it is the last lifecycle event, and f(FORK) = forked.

(2) The new chain C′ starts with e′_REG, a REGISTER event. By Theorem 1, δ(C′) depends only on the last lifecycle event in C′. If C_new = ∅, the last (only) lifecycle event is REGISTER, so δ(C′) = f(REGISTER) = provisional. Any subsequent lifecycle events in C_new are governed by Theorem 1 independently of C. The two derivations operate on disjoint event sequences. ∎

#### C.6.4 Theorem 3 (Monotonicity of Status Transitions)

**Statement.** Let C be a well-formed chain with s = δ(C). For any event e_new, the status transition from s to s′ = δ(C ⊕ [e_new]) is a valid transition according to the transition matrix (Table 3) if and only if e_new.τ is a lifecycle event and the corresponding status change appears in the matrix.

*Proof sketch.* If e_new is a non-lifecycle event, then by Corollary 1, s′ = s and no transition occurs (identity is always a valid "transition"). If e_new is a lifecycle event, then by Theorem 1, s′ is determined by e_new.τ via f. The mapping f yields exactly the target states defined in the transition matrix. Therefore the transition is valid if and only if the corresponding entry exists. ∎

#### C.6.5 Theorem 4 (Confidence Boundedness)

**Statement.** For any well-formed EventChain C, γ(C) ∈ [0, 1].

*Proof sketch.* If C contains no VALIDATE events, then V = [] and γ(C) = 0.0 by definition. Otherwise, c_base = max(0.5, mean(·)) ≥ 0.5. Each per-actor confidence φ(a, V) ∈ [0, 1] by construction (payload validation enforces this bound), so their mean is also in [0, 1]. The return value is min(1.0, c_base + 0.05 × (N_vals − 1)). The increment is non-negative, so the expression inside min is at least c_base ≥ 0.5. The min with 1.0 caps the result at 1.0. Therefore γ(C) ∈ [0, 1]. ∎

#### C.6.6 Theorem 5 (Confidence Monotonicity under Independent Validation)

**Statement.** Let C be a well-formed chain with γ(C) = c. Let e_new be a VALIDATE event from an actor a ∉ actors(V). Then γ(C ⊕ [e_new]) ≥ c, with strict inequality if c < 1.0.

*Proof sketch.* Let A = actors(V) and A′ = A ∪ {a}. Since a ∉ A, we have N′_vals = |A′| = |A| + 1 = N_vals + 1. The new base confidence is:

c′_base = max(0.5, (N_vals · μ + γ) / (N_vals + 1))

where μ is the previous mean of per-actor maxima and γ = e_new.p.confidence ∈ [0, 1]. The new returned confidence is c′ = min(1.0, c′_base + 0.05 · N_vals). The previous confidence was c = min(1.0, c_base + 0.05 · (N_vals − 1)). The validator count increment adds 0.05 to the bonus term. Even if c′_base ≤ c_base (which can occur if γ < μ), the additional 0.05 ensures c′ ≥ c. If c < 1.0, the min is not binding and c′ > c. ∎

#### C.6.7 Corollary 2 (Bounded Confidence Aggregation per Distinct Actor)

**Statement.** A single actor cannot increase confidence by submitting multiple VALIDATE events. Full Sybil resistance—preventing an adversary from creating arbitrarily many distinct actor identities to inflate confidence—requires cryptographic actor authentication (Phase 3; Section 3.7).

*Proof sketch.* By definition of φ(a, V), only the highest confidence value per actor is retained. A subsequent VALIDATE event from the same actor either (i) has lower confidence, leaving φ(a, V) unchanged, or (ii) has higher confidence, replacing the previous value. In neither case does N_vals increase, so the bonus term 0.05 × (N_vals − 1) is unchanged. Therefore the actor cannot increase γ(C) by submitting additional validations. ∎

#### C.6.8 Theorem 6 (Status–Confidence Consistency)

**Statement.** For any well-formed EventChain C, if δ(C) = validated, then γ(C) ≥ 0.5.

*Proof sketch.* If δ(C) = validated, then by Theorem 1, the last lifecycle event in C is VALIDATE. Therefore C contains at least one VALIDATE event, so V ≠ ∅ and γ(C) does not return 0.0. By definition of c_base, we have c_base = max(0.5, mean(·)) ≥ 0.5. By Theorem 4, the final returned value is at least c_base ≥ 0.5. ∎

### C.7 Cryptographic Integrity as a Predicate

The integrity verification procedure VerifyIntegrity(C) used in experiments E1 and E5 can be stated as a predicate over chains:

VerifyIntegrity(C) ≡ WF(C) ∧ ⋀ᵢ₌₂^|C| (eᵢ.h_prev = eᵢ₋₁.h) ∧ ⋀ᵢ₌₁^|C| (eᵢ.h = SHA-256(Canon(eᵢ)))

This predicate is *decidable* in O(n) time for a chain of length n, and *falsifiable* by any of the attack classes in Table 13: payload tampering breaks the hash equality for the tampered event; reordering breaks the h_prev linkage at the swap boundary; deletion breaks the linkage at the deletion point; and replay (inserting an event from a different chain) breaks the h_prev linkage because the replayed event's h_prev does not equal the hash of the preceding event in the target chain.

### C.8 Summary of Formal Guarantees

Table C.1 summarises the formal properties proved in this appendix and their correspondence to the main paper theorems.

| Property | Formal Statement | Paper Reference |
|----------|-----------------|-----------------|
| Determinism | δ is a total function | Theorem 1 |
| Status stability | Non-lifecycle events are δ-neutral | Corollary 1 |
| Fork confluence | δ(C_fork) = forked ∧ δ(C′) = provisional | Theorem 2 |
| Transition monotonicity | δ respects the transition matrix | Theorem 3 |
| Confidence boundedness | γ(C) ∈ [0, 1] | Theorem 4 |
| Confidence monotonicity | Independent validators increase γ | Theorem 5 |
| Bounded aggregation | Same-actor duplicates are γ-neutral | Corollary 2 |
| Status–confidence consistency | δ(C) = validated ⇒ γ(C) ≥ 0.5 | Theorem 6 |
