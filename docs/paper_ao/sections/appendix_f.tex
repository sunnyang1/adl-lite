...
The mapping ADL Lite $\rightarrow$ RDF-star is \emph{lossy} in one direction: RDF-star discards the cryptographic hash chain (it captures the \emph{semantics} of events but not their \emph{integrity evidence}). However, a \emph{reversible} mapping is possible by extending the RDF-star representation with provenance metadata:
