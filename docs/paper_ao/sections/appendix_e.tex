...
\textbf{Key lemma (Total order of $C_{\text{life}}$).} Because $\Phi(C)$ enforces cryptographic linkage (WF$_3$), every event in $C$ has a unique predecessor $e_{\text{prev}}$ with hash $e.h_{\text{prev}} = e_{\text{prev}}.h$. The transitive closure of this linkage induces a total order on all events, and hence the filtered subsequence $C_{\text{life}} = \operatorname{filter}(C, \Sigma_{\text{life}})$ inherits this total order.
...
\textbf{Proof strategy.} Case analysis on the parent status $s$ with direct application of the LUB semantics to the parent, and direct derivation for the child.
...
\begin{enumerate}[label=Step~\arabic*:,leftmargin=2em]
    ...
    \item Case $s = \textit{deprecated}$: $\textit{deprecated}$ is incomparable to $\textit{forked}$ in the status machine; however, the ActionExecutor precondition system forbids fork from deprecated, so this case is unreachable for well-formed chains.
...
\begin{enumerate}[label=(A\arabic*),leftmargin=2em]
    ...
    \item The ActionExecutor precondition system enforces the status machine edges.
...
\begin{enumerate}[label=(A\arabic*),leftmargin=2em]
    ...
    \item The ActionExecutor precondition system enforces that every \textsc{Validate} event carries confidence $c \in [0, 1]$.
...
\begin{enumerate}[label=(A\arabic*),leftmargin=2em]
    ...
    \item The ActionExecutor precondition system enforces that every \textsc{Validate} event carries confidence $c \geq 0.5$.
...
\textbf{Key lemma (Uniqueness of \textsc{Validate} for validated status).} The only lifecycle event type that maps to $\textit{validated}$ under $f$ is \textsc{Validate}. Formally, $f^{-1}(\{\textit{validated}\}) = \{\textsc{Validate}\}$. This follows directly from the definition of $f$ (A2).
...
\textbf{Proof strategy.} Direct proof by lifting the algebraic properties of $\max$ from pairs to finite sets via structural induction on the cardinality of the set.
...
\textbf{Assumptions.}
\begin{enumerate}[label=(A\arabic*),leftmargin=2em]
    \item $C$ is well-formed: $\text{WF}(C)$ holds, satisfying all 13 well-formedness conditions (Definition~5).
    ...
    \item The cryptographic hash function SHA-256 is collision-resistant and the canonicalisation function $\text{Canon}$ is deterministic.
\end{enumerate}

\textbf{Proof strategy.} Direct proof by exhaustive verification of each well-formedness condition $\text{WF}_1$ through $\text{WF}_{13}$ for the extended chain $C'$.
...
\begin{enumerate}[label=Step~\arabic*:,leftmargin=2em]
    ...
    \item $\text{WF}_6$ (Non-null actor): The ActionExecutor rejects events with empty actor fields ($a = \text{null}$ or $a = \text{``''}$) as part of payload validation. Therefore $e_{\text{new}}.a \neq \text{null}$ and $e_{\text{new}}.a \neq \text{``''}$.
    ...
    \item $\text{WF}_9$ (Scope ACL): The ActionExecutor validates that every event's actor has a valid scope prefix (\texttt{public}, \texttt{private/...}, \texttt{user/...}, or \texttt{shared/...}). All existing events in $C$ satisfy scope ACL by $\text{WF}_9(C)$, and $e_{\text{new}}$ is checked at append time. Therefore $C'$ satisfies scope ACL.
    \item $\text{WF}_{10}$ (Signature verification): Events carrying an Ed25519 signature are verified against their SHA-256 hash using the event's public key; events without signatures are exempt. All existing events in $C$ satisfy signature verification by $\text{WF}_{10}(C)$, and $e_{\text{new}}$ is verified by the ActionExecutor if it carries a signature. Therefore $C'$ satisfies signature verification.
    \item $\text{WF}_{11}$ (Confidence clamped): The ActionExecutor enforces $e_{\text{new}}.\text{confidence} \leq \text{MAX\_SCALED}$ as part of the valid-append check. All existing events in $C$ are bounded by $\text{WF}_{11}(C)$. Therefore $C'$ satisfies confidence clamping.
    \item $\text{WF}_{12}$ (SHACL constraints and status transitions): The ActionExecutor enforces SHACL shape validation and validates status transitions against the ontology registry before appending. Therefore $\text{WF}_{12}(C')$ holds.
    \item $\text{WF}_{13}$ (Lifecycle monotonicity, collusion resistance, and synthetic tagging): The ActionExecutor enforces monotonic lifecycle transitions (no backward edges), minimum distinct validators ($N_{\min}$) for collusion resistance, and synthetic-event tagging for parser-reconstructed events. Therefore $\text{WF}_{13}(C')$ holds.
\end{enumerate}

Since all 13 well-formedness conditions hold for $C'$, we conclude $\text{WF}(C')$.

\textbf{Complexity.} Each $\text{WF}_i$ check is $O(1)$ except for three cases: $\text{WF}_5$ (distinctness) is $O(n)$ with a hash set; $\text{WF}_8$ (schema validation) is $O(|p|)$ where $|p|$ is the payload size; and $\text{WF}_{10}$ (signature verification) is $O(1)$ per signature. The total append validation is $O(n + |p|)$.
\end{proof}
...
The nine theorems form a dependency graph. Theorem~\ref{thm:determinism} (foundational) $\to$ Theorem~\ref{thm:confluence} (fork) $\to$ Theorem~\ref{thm:monotonicity-status} (preconditions) is the first chain. Theorem~\ref{thm:boundedness} (independent) $\to$ Theorem~\ref{thm:monotonicity-confidence} (cap argument) $\to$ Theorem~\ref{thm:consistency} (status--confidence) is the second. Theorem~\ref{thm:wf-preservation} (Well-Formedness Preservation) is independent. The optional CRDT property (Theorem~\ref{thm:crdt-convergence-appendix}) and the G-Set corollary depend on Theorem~\ref{thm:determinism} and $\Phi$ respectively. Together, these proofs establish that ADL Lite's derivation semantics is deterministic, fork-deterministic, transition-monotonic, bounded, confidence-monotonic, status--confidence consistent, and well-formedness-preserving.

%-------------------------------------------------------------------------------
\subsection{Randomised Trace-Checker Artifact (E25)}
%-------------------------------------------------------------------------------
\label{subsec:trace-checker}

A randomized trace-checker (Experiment E25, \texttt{experiments/proof\_trace\_checker.py}) generates 10,000 synthetic EventChains of length 2--100 and verifies Theorems 1--7 empirically. All 10,000 traces passed all checks (100\% pass rate). E25 complements the Coq machine proofs (T1--T7, T9) with property-based empirical validation; it does not replace the deductive proofs. All six previously stubbed axioms in \texttt{Chain.v} (precondition evaluation, SHACL constraints, status transitions, lifecycle monotonicity, collusion resistance, and synthetic tagging) have been replaced with their full definitions and preservation proofs, and abstract Ed25519/SHA-256 cryptographic primitives have been introduced in \texttt{Crypto.v}. The Coq development now contains zero remaining \texttt{Admitted} lemmas.

%-------------------------------------------------------------------------------
\subsection{Sensitivity Analysis: Confidence Aggregation Parameters}
%-------------------------------------------------------------------------------

Table~\ref{tab:sensitivity} reports $\gamma_{\text{agg}}(C)$ under three $(\beta, c_{\min})$ combinations for four validator-confidence scenarios (three validators each). Here, $c_{\text{base}} = \max(c_{\min}, \text{mean}(\phi))$ and $\gamma_{\text{agg}}(C) = \min(1.0, c_{\text{base}} + \beta \times (N_{\text{vals}} - 1))$.
...
Theorems~\ref{thm:boundedness}--\ref{thm:consistency} hold for all entries. Boundedness follows because $\gamma_{\text{agg}}(C) \in [0,1]$ by the cap. Monotonicity holds because $\beta > 0$. Status--confidence consistency holds because $c_{\min} \geq 0.3$ ensures any validated concept has $c_{\text{base}} \geq c_{\min}$. Strict monotonicity is preserved for all $\beta > 0$ under the precondition $c \geq c_{\text{base}}$.
...
\textbf{Type safety.} Type mismatches (e.g., comparing a string with GT) are handled by the \texttt{otherwise} clause: the comparison returns \texttt{false} rather than raising an exception. This is a deliberate design choice: the precondition language is a guard, not a general-purpose programming language, and a type mismatch is treated as a failed guard. The Pydantic payload validation layer (outside the precondition language) ensures that type-correct values are passed to the comparators. The \texttt{otherwise} clause serves as a defence-in-depth mechanism for direct API access that bypasses Pydantic.

\textbf{No external references.} The precondition language is closed: \texttt{lookup(f, sigma)} references only the snapshot \texttt{sigma} derived from the local chain $C$. There is no rule for \texttt{lookup(f, sigma)} where $f$ references an external chain, database, or network resource. This local-scope restriction ensures that precondition evaluation is referentially transparent, deterministic, and executable without consensus or network access.
