...
The \texttt{neo4j\_adapter.py} module (148 lines) provides a pluggable graph backend implementing the same interface as the NetworkX DiGraph for the WarmIndex. It supports graph operations including \texttt{add\_edge}, \texttt{bfs}, \texttt{\_\_contains\_\_}, \texttt{node\_count}, and \texttt{close}. The adapter supports dual-write mode, maintaining synchronised SQLite and Neo4j stores for query flexibility, and provides a \texttt{rebuild\_from\_relations} method for graph reconstruction. Configuration is provided via environment variables, enabling deployment-specific tuning without code changes.

\subsection{CRDT Multi-Way Merge Generalisation}\label{app:crdt-multiway}

The \texttt{merge\_event\_chains(*chains)} function has been generalised to support N$\geq$3 chain merging via \texttt{functools.reduce} over the \texttt{\_merge\_two\_chains} helper, maintaining full backward compatibility with the 2-chain case. This enables concurrent merge of multiple branches without pairwise reduction, improving both performance and semantic clarity for multi-agent scenarios. New proof functions \texttt{prove\_multi\_way\_merge()} and \texttt{prove\_event\_chain\_multi\_way\_merge()} establish CRDT convergence properties (Theorem~9) for arbitrary numbers of concurrent branches. The multi-way merge preserves the established properties: commutability, associativity, idempotence, and deterministic state derivation.

\subsection{Multi-Tenant Isolation and Usage Metering}\label{app:multi-tenant}

Implemented in v0.6.0-alpha, the multi-tenant slice provides per-tenant isolation and usage governance for multi-tenant deployments. \texttt{tenant.py} introduces \texttt{TenantContext} and a \texttt{require\_tenant} FastAPI dependency that resolves the tenant for each request and delegates per-tenant \texttt{ConsensusEngine} persistence to the API layer. \texttt{metering.py} maintains persistent per-tenant, per-period counters (API calls and registered entities) in a SQLite \texttt{usage\_meter} table with daily/monthly windows and CSV/JSON export. \texttt{quota.py} (R12) enforces per-tenant quota policies via a \texttt{check\_quota} dependency that raises HTTP~429 when a tenant exceeds its limit, chained as \texttt{require\_tenant} $\to$ \texttt{check\_quota} $\to$ \texttt{meter\_api\_call} $\to$ endpoint. This gives each tenant an isolated, metered, and quota-bounded view of the registry without cross-tenant state leakage.

\subsection{Trust Model}\label{app:trust-model}

Implemented in v0.6.0-alpha, the trust model (\texttt{trust\_model.py}) operationalizes the adversarial baseline checks B1--B4 as executable validators. \texttt{ConsensusConfig} is the single source of truth for minimum-validator thresholds, with \texttt{resolve()} enforcing production safety floors (e.g., at least two distinct validators in strict mode) and emitting warnings in dev mode. \texttt{TrustValidator.validate\_event\_chain()} checks B1 (N\textsubscript{min} threshold on distinct validators), B2 (DID binding of actor keys), B3 (basic Sybil resistance), and B4 (validator diversity, a Phase-1 placeholder). This makes the trust boundary described in Section~\ref{subsec:trust-model} directly testable rather than purely design-time.
