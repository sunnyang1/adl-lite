...
This appendix provides the full natural-language proofs, well-formedness axioms, and sensitivity analysis that were compressed from Section~\ref{subsec:formal-semantics}.
...
\textbf{Theorem~\ref{thm:monotonicity} (Transition Monotonicity).} For any well-formed $C$ and event $e_{\text{new}}$, the transition from $s = \delta(C)$ to $s' = \delta(C \oplus [e_{\text{new}}])$ is valid iff $e_{\text{new}}.\tau \in \Sigma_{\text{life}}$.
\begin{proof}
...
\textbf{Remark (Collusion vulnerability).} Theorem~5 assumes non-colluding actors. As demonstrated in the empirical boundary experiment E14 (Section~\ref{subsec:e14}), a single colluding actor can drive $\gamma_{\text{agg}}(C)$ to $0.99$. This vulnerability is a known Phase~1 limitation (L9, Section~\ref{subsec:limitations}), addressed by the calibration layer (\S\ref{subsec:calibration}) and staking-based validation (FW9, Section~\ref{subsec:future-work}).
...
\textbf{Theorem~\ref{thm:crdt-convergence} (CRDT Convergence).} For any well-formed concurrent branches $B_1, B_2$ derived from the same genesis event, the LWW-Set merge satisfies:
\begin{align}
