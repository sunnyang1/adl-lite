...
\textbf{Remark (Collusion vulnerability).} Theorem~5 assumes non-colluding actors. As demonstrated in the empirical boundary experiment E14 (Section~\ref{subsec:boundary-experiments}), a single colluding actor can drive $\gamma_{\text{agg}}(C)$ to $0.99$. This vulnerability is a known Phase~1 limitation (L9, Section~\ref{subsec:limitations}), addressed by the calibration layer (\S\ref{subsec:calibration}) and staking-based validation (FW9, Section~\ref{subsec:future-work}).
