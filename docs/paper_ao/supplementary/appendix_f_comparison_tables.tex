...
This appendix provides the full comparative capability-audit discussion that was compressed from Section~\ref{subsec:comparative-analysis}.
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{Deterministic aggregation threshold check.} Determine whether a concept has reached a configurable confidence threshold (e.g., $\gamma(C) \geq 0.7$) and which validators contributed. Measure: computational cost of threshold evaluation.
\end{enumerate}

\subsection{Comparison Results}

Table~\ref{tab:comparative-eval-full} presents the qualitative audit task comparison. Table~\ref{tab:comparative-benchmark-full} supplements this with quantitative benchmarks measured on identical hardware (Apple M2, macOS 14, Python 3.10) via the E12 experiment.
...
Deterministic aggregation threshold & $\times$ (no built-in aggregation) & $\times$ (no built-in aggregation) & $\checkmark$ ($\gamma(C)$ with quorum rules) \\
...
\textbf{Head-to-head benchmark.} A direct quantitative benchmark against nanopublication and PROV-O pipelines on identical audit tasks is deferred to Phase~3, when ADL Lite's Ed25519 authentication layer will be comparable to Trusty URIs' RSA signatures. Until then, the comparison in Table~\ref{tab:comparative-benchmark-full} reports measured ADL Lite figures against published estimates for the baselines.
