...
\textbf{Key findings.} ADL Lite achieves the lowest latency ($\approx$0.0 ms, pure in-memory operations) and competitive LOC (27 total, or 6--7 per task) because state derivation ($\delta$, $\gamma$) is built-in. Nanopub has the lowest LOC (21 total, 5--6 per task) but lower audit completeness (0.82) due to the lack of native deprecation. PROV-O requires more LOC (50 total) because every entity, activity, and relation must be explicitly declared with \texttt{QualifiedName} objects. Git-only has the highest latency (21.25 ms) because each state change triggers actual file I/O and a Git commit, but achieves full audit completeness (1.0) because the Git log is a complete tamper-evident audit trail.
...
\begin{table}[htbp]
\centering
\caption{Empirical governance cost benchmark (E19): ADL Lite vs. baselines on 4 governance tasks. \textbf{All values are measured via actual execution on identical hardware.}}
...
\textbf{Key findings.} (i)~ADL Lite achieves 22,987 events/s at $10^6$ scale, confirming the linear throughput claim from E6. The measured time (87.0~s for 2$\times$$10^6$ events) is higher than the E6b projection (47.7~s for 1$\times$$10^6$ events) because E6b was based on CSV import throughput (20,847 events/s), whereas the E19 scale test creates independent EventChain objects with full Pydantic materialization overhead. (ii)~Nanopub has the highest triple throughput (51,331 triples/s) but lower audit completeness (0.82) due to the lack of native deprecation. (iii)~PROV-O's throughput is lower (13,673 events/s) because every entity and activity requires a \texttt{QualifiedName} and explicit relation declaration; the projected $10^6$ time (146.3~s) is $\approx$1.7$\times$ ADL Lite. (iv)~Git-only with batch commits (100 concepts per commit) is surprisingly efficient at scale (projected 54.6~s for $10^6$) because batching amortizes the per-commit overhead, but the audit trail is coarser (one commit per 100 concepts, not one per event).
