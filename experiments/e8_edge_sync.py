...
from adl_lite.sync_manager import (
    EdgeNode, SyncManager, SideEffectQueue, QueuedEffect,
)
...
class E8EdgeSync(BaseExperiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        if not offline_ok:
            errors.append(f"Offline: integrity={chain_ok}, length={offline_length}, queued={queued_count}")
        ...
        edge_chain.append(Event(concept_id="push-test", event_type=EventType.REGISTER, actor="genesis"))
        edge_chain.append(Event(concept_id="push-test", event_type=EventType.REGISTER, actor="edge"))

        center_chain = EventChain(concept_id="push-test")
        center_chain.append(Event(concept_id="push-test", event_type=EventType.REGISTER, actor="genesis"))
        ...
        center_chain.append(Event(concept_id="push-test", event_type=EventType.VALIDATE, actor="center"))
        ...
        # ===== TEST 5: Reconnect drain =====
        edge2 = EdgeNode(concept_id="reconnect-test", node_id="edge-2")
        edge2.go_offline()
        edge2.record_event(EventType.REGISTER, {"note": "offline"},
                           side_effects=["lark_announce"])
        assert edge2.queue.pending == 1

        result = edge2.go_online()  # No center chain provided
        drain_ok = result["chains_synced"] == False and result["effects_drained"] >= 0
        results.append({
        ...
        all_ok = offline_ok and merged_ok and push_ok and pull_ok and drain_ok
